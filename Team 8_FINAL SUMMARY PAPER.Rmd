...
Multivariate Adaptive Regression Splines (MARS), is an algorithm designed for multivariate non-linear regression problems.

Multivariate refers to the presence of multiple input variables, while nonlinear refers to the absence of a linear relationship that is, one that can be expressed as a straight line between the input variables and the target variable. 

The MARS algorithm involves discovering a set of simple piece wise linear functions that characterize the data and using them in aggregate to make a prediction. In a sense, the model is an ensemble of linear functions.

**MARS method works as follows:**
...
We have chosen MARS method so that we can naturally handle mixed types of predictors (quantitative and qualitative) and as it also considers all possible binary partitions of the categories for a qualitative predictor into two groups.
...
```{r}
#Plotting the Mars Method Results
plot(ev)
```

**Results:**
In the output, firstly we find out the Generalized cross validation (GCV), number of subset models when the variable occurs (nsubsets) and the residual sum of squares (RSS) and we perform the variable importance then we can see that by using the MARS method there are 4 important features which are Total_Years, engine_capacity, drivetrain and the Unknown_feature_6. 
...
**Why we are using Lasso regression?**
...
LASSO is short for Least Absolute Shrinkage and Selection Operator which is used for both regularization and model selection. This method is frequently used to choose a subset of variables. It outperforms other regression models in terms of prediction accuracy. Lasso Regularization also improves model interpretation.
...
model_lasso  <- train(price_usd~transmission + color + odometer_value + engine_type +
    engine_capacity + body_type + drivetrain + state + number_of_photos +
    feature_7 + feature_3 + feature_6 + feature_5 +
    year_produced + manufacturer_name,data = new_df,
...
We have chosen to perform ridge regression is because it protects the model from overfitting and it does not need unbiased estimators. There is only enough bias to make the estimates reasonably reliable approximations to the true population values.
...
**Results:**
Using Lasso and Ridge regression we got to know that there is no improvement in the Linear regression model after adding L1 or L2 regularization term in it.

## SVM Regressor

**Why we are using SVM Regressor?**

As the model is not only restricted to a linear model(as we have many kernel options), we have better performance in this model. At the same time SVR gives us flexibility to control the allowable errors – which is a big plus!
Here we are using the best kernel type that gives us high accuracy
...
The objective function of SVR is to minimize the coefficients and maximizing the margin.

We can tune epsilon to gain the desired accuracy level of our model.

**Our new objective function and constraints are as follows:**

By default, the SVR model uses epsilon = 0.1 and cost=1.0 to build the model.

If we don’t define any kernel in particular, the model automatically chooses best kernel on its own. But we have used a ‘Radial kernel’
...
RF can be used to solve both Classification and Regression tasks.

The name “Random Forest” comes from the Bagging idea of data randomization (Random) and building multiple Decision Trees (Forest).

Overall, it is a powerful algorithm that limits the disadvantages of a Decision Tree model.


**Why we are using Random forest?:**

As it is using the bagging technique, we get model with low bias and low variance.

It almost does not overfit due to subset and feature randomization.
...
**Random forest is giving very good accuracy and it is minimizing the error term with high margin.** 
...
**We performed hyperparameter tuning and As we got best max.depth = 4 and n rounds of 600, we are getting the max test rsquare score 0.9273930 which is very good model.**
