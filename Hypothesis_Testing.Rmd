...
Anova test for color body type 
```{r}
df_aov_1 = aov(price_usd ~  color , df)
summary(df_aov_1)
```

```{r}
df_aov_4 = aov(price_usd ~  body_type , df)
summary(df_aov_4)
```
```{r}
df_aov_2 = aov(price_usd~  engine_fuel , df)
summary(df_aov_2)
```

```{r}
df_aov_3 = aov(price_usd~  engine_type , df)
summary(df_aov_3)
```

t-test for transmission type

```{r T test, echo=TRUE}
summary(df$transmission)
t_auto = subset(df, transmission == "automatic")
t_mechanical = subset(df, transmission == "mechanical")
t.test(x = t_auto$price, y = t_mechanical$price, conf.level = 0.99)
```
